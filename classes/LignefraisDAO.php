...
function nb_deplacements($id_club){
  $sql = "select ligne_frais.id_ligne_frais from ligne_frais, note_frais where ligne_frais.id_note_frais = note_frais.id_note_frais and id_club=:id_club";
    $params = array(
      ":id_club"=>$id_club
    );
    $sth = $this->executer($sql, $params);
  ...
  function montant_frais($id_club){
    $sql = "select sum(indemnite.tarif_kilometrique * km_parcourus + cout_peage+cout_repas+cout_hebergement) as mt
    from indemnite, ligne_frais, note_frais
    WHERE note_frais.id_note_frais = ligne_frais.id_note_frais
    AND note_frais.annee = indemnite.annee
    AND note_frais.id_club = :id_club";
      $params = array(
        ":id_club"=>$id_club
      );
      $sth = $this->executer($sql, $params);
